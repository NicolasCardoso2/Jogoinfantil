...
const SYLLABLE_AUDIO_KEYS = [
  // Sílabas da Família F
  'word_FA','word_FE','word_FI','word_FO','word_FU'
];
...
export function loadAllGameAudio(loader) {
  ...
  // Música de fundo da Fase 4
  loader.audio('bg_music_fase4', 'assets/sons/fase4.mp3');
}

export default class Fase4Atividades extends Phaser.Scene {
  constructor() {
    super({ key: 'fase4_atividades' });
    this.wordAudioLetter = 'f'; // Fase 4 = Fazenda do F (palavras começam com F)
    ...
    this.phaseBestStars = (_save.progress?.F?.bestStars) || 0;

    // Paleta de cores suaves e neutras (tema fazenda)
    this.COLOR = {
  ...
  // =========================
  // PRELOAD - Carrega todas as imagens da Fase 4 (Fazenda do F)
  // =========================
  preload() {
    ...
    // Fundo da fazenda
    this.load.image('bg_Fazenda', 'assets/bg_Fazenda.png');
    ...
    // Imagens com F
    this.load.image('fada',     'assets/imag sem letra/FADA.png');
    this.load.image('farol',    'assets/imag sem letra/FAROL.png');
    this.load.image('fazenda',  'assets/imag sem letra/FAZENDA.png');
    this.load.image('feijao',   'assets/imag sem letra/Feijao.png');
    this.load.image('festa',    'assets/imag sem letra/FESTA.png');
    this.load.image('fita',     'assets/imag sem letra/FITA.png');
    this.load.image('foca',     'assets/imag sem letra/FOCA.png');
    this.load.image('fogo',     'assets/imag sem letra/FOGO.png');
    this.load.image('folha',    'assets/imag sem letra/FOLHA.png');
    this.load.image('futebol',  'assets/imag sem letra/FUTEBOL.png');
    ...
    // Gera texturas de sílabias da família F se não existirem
    this.load.on('complete', () => {
      F_SYLLS.forEach(syll => {
  ...
  // Fala o nome de uma figura (bola, bebe, dado, etc.)
  playWordAudio(imageKey) {
    ...
    // Se já for uma chave completa (ex: "word_f_fada"), toca direto
    ...
    // Palavras da família F e distratoras comuns
    const F_WORDS = new Set([
      'fada','farol','fazenda','feijao','festa',
      'fita','foca','fogo','folha','futebol'
    ]);
    ...
    if (F_WORDS.has(k)) {
      audioKey = `word_f_${k}`;       // ex: "fada" -> "word_f_fada"
  ...
  // Fala o áudio de ajuda da atividade (help_b_a1_..., help_b_a2_..., etc.)
  ...
  playTitleAudio(activityNumber) {
    const map = {
      ...
      3: 'title_a3_figuras_com_letra_F',               // Figuras com F
  ...
  // Reseta o progresso da fase para começar sempre do início
  showPhaseCompletedModal() {
    ...
    const estrelasBest = this.phaseBestStars || parseInt(localStorage.getItem('fase4Stars') || '0', 10) || 0;
    const estrelas     = Math.max(estrelasNow, estrelasBest);

    let badgeKey = null;
    if (estrelas >= 3 && this.textures.exists('insignia_f_3')) badgeKey = 'insignia_f_3';
    else if (estrelas === 2 && this.textures.exists('insignia_f_2')) badgeKey = 'insignia_f_2';
    else if (estrelas === 1 && this.textures.exists('insignia_f_1')) badgeKey = 'insignia_f_1';
    ...
    const btnReplay = track(this.createModernButton(
      ...
      () => {
        cleanup();
        // Volta para a primeira atividade
        this.currentActivity = 1;
        this.resetPhaseErrors();
        this.showActivity(1);
      }
    ));
  ...
  createForestBackground() {
    ...
    if (this.textures && this.textures.exists('bg_Fazenda')) {
      const tex  = this.textures.get('bg_Fazenda').get();
      ...
      this.add.image(W/2, H/2, 'bg_Fazenda').setScale(s).setDepth(-20);
      return;
    }
    // se não carregar, deixa um fundo verde simples (tema fazenda)
...
markPermanentSuccess(target, W, H, color = this.COLOR.SUCCESS) {
  ...
  stopBackgroundMusic() {
    if (this.bgMusic) {
      try {
        console.log('Fase4: parando música de fundo...');
        this.bgMusic.stop();
        this.bgMusic.destroy();
        this.bgMusic = null;
      } catch (e) {
  ...
  marcarFaseCompleta() {
    ...
    // Marca que, ao voltar para o mapa, a fase 4 deve animar a ins?gnia
    localStorage.setItem('fase4ShouldAnimate', '1');
    return estrelas;
  }
  ...
  doClearActivity() {
    ...
    this.a5i = 0;
    this.a6Placed = 0; // reset "Separe FA/FO"
    this.a9Index = 0;
  ...
  showEndModalPortrait(nextFn) {
    ...
    if (scene.textures.exists('popup_next')) {
      ...
      cta.on('pointerdown', () => {
        // Proteção dupla: cooldown global + flag específica do botão
  ...
  this.makeTitle('Que sílaba começa?');
    ...
    // Cobertura balanceada de todas as 5 sílabas F
    const allOptions = [
      { image:'fada',    correct:'FA' },
      { image:'farol',   correct:'FA' },
      { image:'fazenda', correct:'FA' },
      { image:'feijao',  correct:'FE' },
      { image:'festa',   correct:'FE' },
      { image:'fita',    correct:'FI' },
      { image:'foca',    correct:'FO' },
      { image:'fogo',    correct:'FO' },
      { image:'folha',   correct:'FO' },
      { image:'futebol', correct:'FU' },
    ];

    // Garante pelo menos 1 de cada sílaba, depois completa aleatoriamente
    const required = ['FA','FE','FI','FO','FU'];
    const selected = [];
    ...
    this.a1List = selected.map(item => ({
      ...item,
      wrong: Phaser.Utils.Array.GetRandom(['FA','FE','FI','FO','FU'].filter(s => s !== item.correct))
    }));
...
renderA1() {
  ...
  const wordMap = {
    'fada': 'FADA', 'farol': 'FAROL', 'fazenda': 'FAZENDA',
    'feijao': 'FEIJÃO', 'festa': 'FESTA', 'fita': 'FITA',
    'foca': 'FOCA', 'fogo': 'FOGO', 'folha': 'FOLHA', 'futebol': 'FUTEBOL'
  };
  ...
  setupAtividade2() {
    ...
    this._a2Back = a2Back;
  this.makeTitle('Figuras com F');
    this.playTitleAudio(3);

    this.addTutorialButton(
      a2Back,
      'Toque apenas nas imagens cujos nomes começam com a letra F.',
      'help_f_a3_figuras_com_f'
    );

    this.a2Rounds = [
      ["fada","farol","fazenda","feijao","dado","gato"],
      ["foca","fogo","folha","futebol","pato","vela"]
    ];
    this.a2RoundIndex = 0;
    this.renderA2Round();
  }
  ...
  renderA2Round() {
    ...
    this.a2_total = imgs.filter(n => n[0]==='f').length;
    this.a2_hits  = 0;
    ...
    imgs.forEach((key,i)=>{
      ...
      cont._done=false; cont._isF = (key[0]==='f');
  ...
  this.makeTitle('Que palavra é essa?');
    ...
    this.a4List = [
      { image:'fada',    correct:'FADA',    options:['FADA','FOCA','FITA'] },
      { image:'fazenda', correct:'FAZENDA', options:['FAZENDA','FESTA','FOLHA'] },
      { image:'foca',    correct:'FOCA',    options:['FOCA','FOGO','FOLHA'] },
      { image:'futebol', correct:'FUTEBOL', options:['FUTEBOL','FITA','FESTA'] },
      { image:'feijao',  correct:'FEIJÃO',  options:['FEIJÃO','FADA','FAROL'] },
    ];
  ...
  this.a5Title.setFontSize(46);
    ...
    this.a5Rounds = [
      { alvo:'FO', itens:['foca','fogo','folha','fita'] },     // intruso: fita (FI)
      { alvo:'FA', itens:['fada','farol','fazenda','feijao'] }, // intruso: feijao (FE)
      { alvo:'FO', itens:['foca','fogo','folha','futebol'] },   // intruso: futebol (FU)
    ];
    this.a5i=0;
  ...
  setupAtividade5() {
    ...
    this.addTutorialButton(
      a6Back,
      'Arraste cada imagem para o cesto da sílaba correta: FA ou FO.',
      'help_f_a5_separe_fa_fo'
    );
    ...
    this.a6Left  = this.makeBucketPanel(W/2 - 200, bucketY, 'FA', this.COLOR.SECONDARY, 120);
    this.a6Right = this.makeBucketPanel(W/2 + 200, bucketY, 'FO', this.COLOR.PRIMARY, 120);
    const leftZone  = this.a6Left.zone;  leftZone.syll = 'FA';
    const rightZone = this.a6Right.zone; rightZone.syll = 'FO';
  ...
  startA6Round() {
    ...
    const ROUNDS = [
      [ ['FA','fada'], ['FO','foca'] ],                                          // 2 pares  → 4 cartas
      [ ['FA','farol'], ['FU','futebol'], ['FI','fita'] ],                       // 3 pares  → 6 cartas
      [ ['FA','fazenda'], ['FO','fogo'], ['FU','futebol'], ['FE','feijao'] ],    // 4 pares  → 8 cartas
    ];
...
renderA8Round() {
  ...
  this.makeTitle('Ache a figura certa');
    ...
    this.a9Rounds = [
      { word:'FADA',    correct:'fada',    pool:['foca','fita','farol','dado'] },
      { word:'FAZENDA', correct:'fazenda', pool:['festa','fogo','folha','gato'] },
      { word:'FOCA',    correct:'foca',    pool:['fada','fita','farol','pato'] },
      { word:'FUTEBOL', correct:'futebol', pool:['festa','fogo','folha','vela'] },
    ];
    this.a9Index = 0;
    this._a9FirstRound = true;
    this.renderA9Round();
  ...
  getSyllableForKey(key) {
    ...
    return map[key] || 'FA'; // fallback seguro
  }

  // Monte a rodada da A9 com base nas imagens mostradas
  buildA9Round() {
    // escolha 3 imagens do pool
    const pool = ['fada','farol','fazenda','feijao','festa','fita','foca','fogo','folha','futebol'];
...
setupAtividade9() {
  ...
  // ordem pedagógica: FA -> FE -> FI -> FO -> FU
  this._a9Order = ['FA','FE','FI','FO','FU'];
  this._a9Step  = 0;
  this._a9Lock  = false;

  // pool por família (usa os assets que você já carrega)
  this._a9Items = {
    FA: ['fada','farol','fazenda'],
    FE: ['feijao','festa'],
    FI: ['fita'],
    FO: ['foca','fogo','folha'],
    FU: ['futebol'],
