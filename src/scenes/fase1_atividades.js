...
const SYLLABLE_AUDIO_KEYS = [
  'word_BA','word_BE','word_BI','word_BO','word_BU'
];
...
export function loadAllGameAudio(loader) {
  ...
  loader.audio('bg_music_fase1', 'assets/sons/fase1.mp3');
}

export default class Fase1Atividades extends Phaser.Scene {
  constructor() {
    super({ key: 'fase1_atividades' });
    this.wordAudioLetter = 'b'; // Fase 1 = Bosque do B (palavras começam com B)
    ...
    this.phaseBestStars = (_save.progress?.B?.bestStars) || 0;
  ...
  // =========================
  // PRELOAD - Carrega todas as imagens da Fase 1 (Bosque do B)
  // =========================
  preload() {
  ...
  playWordAudio(imageKey) {
    ...
    // Se já for uma chave completa (ex: "word_b_bola"), toca direto
    ...
    // Palavras da família B e distratoras comuns
    const B_WORDS = new Set([
      'bala','balde','banana','bebe','bigode','bode',
      'boia','bola','bota','bule','buzina'
    ]);
    ...
    if (B_WORDS.has(k)) {
      audioKey = `word_b_${k}`;       // ex: "bola" -> "word_b_bola"
  ...
  playTitleAudio(activityNumber) {
    const map = {
      ...
      3: 'title_a3_figuras_com_letra',                 // Figuras com B
...
markPermanentSuccess(target, W, H, color = this.COLOR.SUCCESS) {
  ...
  stopBackgroundMusic() {
    if (this.bgMusic) {
      try {
        console.log('Fase1: parando música de fundo...');
        this.bgMusic.stop();
        this.bgMusic.destroy();
        this.bgMusic = null;
      } catch (e) {
  ...
  marcarFaseCompleta() {
    ...
    // Marca que, ao voltar para o mapa, a fase 1 deve animar a ins?gnia
...
renderA1() {
  ...
  this.makeTitle('Que palavra é essa?');
    ...
    this.a4List = [
      ...
      { image:'banana',  correct:'BANANA',  options:['BANANA','BALA','BULE'] },
      { image:'bola',    correct:'BOLA',    options:['BOLA','BOTA','BULE'] },
      { image:'buzina',  correct:'BUZINA',  options:['BUZINA','BOTA','BULE'] },
...
renderA8Round() {
  ...
  this.makeTitle('Ache a figura certa');
    ...
    this.a9Rounds = [
      ...
      { word:'BOTA',    correct:'bota',    pool:['bola','bule','balde','bebe'] },
      { word:'BULE',    correct:'bule',    pool:['bola','bota','bala','bebe'] },
      { word:'BALA',    correct:'bala',    pool:['bola','bota','boia','bebe'] },
      { word:'BANANA',  correct:'banana',  pool:['bala','bota','bola','bebe'] },
      { word:'BUZINA',  correct:'buzina',  pool:['bule','bola','bota','bebe'] },
    ];
    this.a9Index = 0;
    this._a9FirstRound = true;
    this.renderA9Round();
