...
const SYLLABLE_AUDIO_KEYS = [
  'word_DA','word_DE','word_DI','word_DO','word_DU'
];
...
export function loadAllGameAudio(loader) {
  ...
  // Música de fundo da Fase 1
  loader.audio('bg_music_fase3', 'assets/sons/fase3.mp3');
}

export default class Fase3Atividades extends Phaser.Scene {
  constructor() {
    super({ key: 'fase3_atividades' });
    this.wordAudioLetter = 'd'; // Fase 3 = Dinossauros do D (palavras começam com D)
    ...
    this.phaseBestStars = (_save.progress?.D?.bestStars) || 0;
  ...
  // =========================
  // PRELOAD - Carrega todas as imagens da Fase 3 (Dinossauros do D)
  // =========================
  preload() {
    ...
    // Fundo do Mundo Dino
    this.load.image('bg_Dinosauros', 'assets/bg_Dinosauros.png');
    ...
    // Imagens da família D
    // Família DA
    this.load.image('dado',    'assets/imag sem letra/DADO.png');
    this.load.image('dama',    'assets/imag sem letra/DAMA.png');
    this.load.image('danca',   'assets/imag sem letra/DANCA.png');
    // Família DE
    this.load.image('dedo',    'assets/imag sem letra/DEDO.png');
    this.load.image('dente',   'assets/imag sem letra/DENTE.png');
    this.load.image('desenho', 'assets/imag sem letra/DESENHO.png');
    // Família DI
    this.load.image('diamante','assets/imag sem letra/DIAMANTE.png');
    this.load.image('dinamite','assets/imag sem letra/DINAMITE.png');
    this.load.image('dinheiro','assets/imag sem letra/DINHEIRO.png');
    this.load.image('dino',    'assets/imag sem letra/DINO.png');
    // Família DO
    this.load.image('doce',    'assets/imag sem letra/DOCE.png');
    this.load.image('doente',  'assets/imag sem letra/DOENTE.png');
    this.load.image('dois',    'assets/imag sem letra/DOIS.png');
    this.load.image('domino',  'assets/imag sem letra/DOMINO.png');
    this.load.image('dormir',  'assets/imag sem letra/DORMIR.png');
    // Família DU
    this.load.image('duende',  'assets/imag sem letra/DUENDE.png');
    this.load.image('ducha',   'assets/imag sem letra/DUCHA.png');

    // Imagens distratoras (não começam com D)
    ...
    // Gera texturas de sílabas da família B se não existirem
    this.load.on('complete', () => {
      D_SYLLS.forEach(syll => {
  ...
  // Fala o nome de uma figura (dado, dama, doce, etc.)
  playWordAudio(imageKey) {
    ...
    // Se já for uma chave completa (ex: "word_d_dado"), toca direto
    ...
    // Palavras da família D e distratoras comuns
    const D_WORDS = new Set([
      'dado','dama','danca','dedo','dente','desenho',
      'diamante','dinamite','dinheiro','dino','doce','doente',
      'dois','domino','dormir','duende','ducha'
    ]);

    const COMMON = new Set(['gato','pato','vela']);

    let audioKey = null;

    if (D_WORDS.has(k)) {
      audioKey = `word_d_${k}`;       // ex: "dado" -> "word_d_dado"
    } else if (COMMON.has(k)) {
      audioKey = `word_all_${k}`;     // ex: "gato" -> "word_all_gato"
    }
  ...
  // Fala uma sílaba (BA, BE, BO...) usando os áudios word_BA, word_BE, etc.
  playSyllableAudio(syllableKey) {
    ...
    // Se já vier como "word_BA", usa direto
    if (!k.startsWith('WORD_')) {
      k = `word_${k}`;                // "BA" -> "word_BA"
    }
  ...
  // Fala o áudio de ajuda da atividade (help_d_a1_..., help_d_a2_..., etc.)
  ...
  playTitleAudio(activityNumber) {
    const map = {
      1: 'title_a1_ache_figura_certa',                 // Ache a figura certa
      2: 'title_a2_qual_silaba_comeca_palavra',        // Que sílaba começa?
      3: 'title_a3_figuras_com_letra_D',               // Figuras com D
      4: 'title_a4_encontre_figura_diferente',         // Ache o intruso
      5: 'title_a5_separe_figuras',                    // Separe as figuras
      6: 'title_a6_que_palavra_e_essa',                // Que palavra é essa?
      7: 'title_a7_monte_a_palavra',                   // Monte a palavra
      8: 'title_a8_jogo_da_memoria',                   // Jogo da memória
      9: 'title_a9_silaba_certa_para_cada_figura'      // Sílaba certa para cada figura
    };
  ...
  showPhaseCompletedModal() {
    ...
    const estrelasBest = this.phaseBestStars || parseInt(localStorage.getItem('fase3Stars') || '0', 10) || 0;
    const estrelas     = Math.max(estrelasNow, estrelasBest);

    let badgeKey = null;
    if (estrelas >= 3 && this.textures.exists('insignia_d_3')) badgeKey = 'insignia_d_3';
    else if (estrelas === 2 && this.textures.exists('insignia_d_2')) badgeKey = 'insignia_d_2';
    else if (estrelas === 1 && this.textures.exists('insignia_d_1')) badgeKey = 'insignia_d_1';
  ...
  createForestBackground() {
    ...
    if (this.textures && this.textures.exists('bg_Dinosauros')) {
      const tex  = this.textures.get('bg_Dinosauros').get();
      ...
      this.add.image(W/2, H/2, 'bg_Dinosauros').setScale(s).setDepth(-20);
      return;
    }
  ...
  createHeader() {
    ...
    const titleText = 'Dinossauros do D';
...
markPermanentSuccess(target, W, H, color = this.COLOR.SUCCESS) {
  ...
  stopBackgroundMusic() {
    if (this.bgMusic) {
      try {
        console.log('Fase3: parando música de fundo...');
        this.bgMusic.stop();
        this.bgMusic.destroy();
        this.bgMusic = null;
      } catch (e) {
  ...
  marcarFaseCompleta() {
    ...
    // Marca que, ao voltar para o mapa, a fase 3 deve animar a ins?gnia
    localStorage.setItem('fase3ShouldAnimate', '1');
    return estrelas;
  }
  ...
  showActivity(n, withIntro=false) {
    ...
    switch (n) {
      ...
      case 3: this.setupAtividade2(); break; // A2 → Toque nas imagens que começam com D
      case 4: this.setupAtividade4(); break; // A4 → Toque o intruso (NÃO começa com DO)
      case 5: this.setupAtividade5(); break; // A5 → Arraste para o cesto correto (DA × DO)
  ...
  doClearActivity() {
    ...
    this.a5i = 0;
    this.a6Placed = 0; // reset "Separe DA/DO"
    this.a9Index = 0;
  ...
  showCompletion(message = "Atividade concluída!") {
  ...
  this.makeTitle('Que sílaba começa?');
    ...
    // Cobertura balanceada de todas as 5 sílabas D
    const allOptions = [
      { image:'dado',    correct:'DA' },
      { image:'dama',    correct:'DA' },
      { image:'danca',   correct:'DA' },
      { image:'dedo',    correct:'DE' },
      { image:'dente',   correct:'DE' },
      { image:'desenho', correct:'DE' },
      { image:'diamante',correct:'DI' },
      { image:'dinamite',correct:'DI' },
      { image:'dinheiro',correct:'DI' },
      { image:'dino',    correct:'DI' },
      { image:'doce',    correct:'DO' },
      { image:'doente',  correct:'DO' },
      { image:'dois',    correct:'DO' },
      { image:'domino',  correct:'DO' },
      { image:'dormir',  correct:'DO' },
      { image:'duende',  correct:'DU' },
      { image:'ducha',   correct:'DU' },
    ];

    // Garante pelo menos 1 de cada sílaba, depois completa aleatoriamente
    const required = ['DA','DE','DI','DO','DU'];
    const selected = [];
    ...
    this.a1List = selected.map(item => ({
      ...item,
      wrong: Phaser.Utils.Array.GetRandom(['DA','DE','DI','DO','DU'].filter(s => s !== item.correct))
    }));
...
renderA1() {
  ...
  const wordMap = {
    'dado': 'DADO', 'dama': 'DAMA', 'danca': 'DANÇA',
    'dedo': 'DEDO', 'dente': 'DENTE', 'desenho': 'DESENHO',
    'diamante': 'DIAMANTE', 'dinamite': 'DINAMITE', 'dinheiro': 'DINHEIRO', 'dino': 'DINO',
    'doce': 'DOCE', 'doente': 'DOENTE', 'dois': 'DOIS', 'domino': 'DOMINÓ', 'dormir': 'DORMIR',
    'duende': 'DUENDE', 'ducha': 'DUCHA'
  };
  ...
  setupAtividade2() {
    ...
    this._a2Back = a2Back;
  this.makeTitle('Figuras com D');
    this.playTitleAudio(3);

    this.addTutorialButton(
      a2Back,
      'Toque apenas nas imagens cujos nomes começam com a letra D.',
      'help_d_a3_figuras_com_d'
    );

    this.a2Rounds = [
      ["dado","dama","dente","dino","gato","pato"],
      ["doce","dormir","duende","vela","dado","gato"]
    ];
    this.a2RoundIndex = 0;
    this.renderA2Round();
  }
  ...
  renderA2Round() {
    ...
    this.a2_total = imgs.filter(n => n[0]==='d').length;
    this.a2_hits  = 0;
    ...
    imgs.forEach((key,i)=>{
      ...
      cont._done=false; cont._isD = (key[0]==='d');
  ...
  this.makeTitle('Que palavra é essa?');
    ...
    this.a4List = [
      { image:'dado',    correct:'DADO',    options:['DADO','DENTE','DAMA'] },
      { image:'dama',    correct:'DAMA',    options:['DAMA','DADO','DENTE'] },
      { image:'dente',   correct:'DENTE',   options:['DENTE','DADO','DAMA'] },
      { image:'dino',    correct:'DINO',    options:['DINO','DENTE','DADO'] },
      { image:'doce',    correct:'DOCE',    options:['DOCE','DENTE','DAMA'] },
    ];
  ...
  this.a5Title.setFontSize(46);
    ...
    // Áudio de ajuda genérico disponível quando necessário
    this.addTutorialButton(
      a5Back,
      'Encontre a imagem que não combina com as outras.',
      'help_a4_ache_intruso'
    ...
    this.a5Rounds = [
      { alvo:'DO', itens:['doce','domino','doente','dama'] },    // intruso: dama
      { alvo:'DA', itens:['dado','dama','dente','dino'] },       // intruso: dino (DE/DI)
      { alvo:'DO', itens:['doce','domino','doente','ducha'] },   // intruso: ducha
    ];
    this.a5i=0;
  ...
  setupAtividade5() {
    ...
    this.addTutorialButton(
      a6Back,
      'Arraste as figuras para o cesto DA ou DO.',
      'help_d_a5_separe_da_do'
    );
    ...
    this.a6Left  = this.makeBucketPanel(W/2 - 200, bucketY, 'DA', this.COLOR.SECONDARY, 120);
    this.a6Right = this.makeBucketPanel(W/2 + 200, bucketY, 'DO', this.COLOR.PRIMARY, 120);
    const leftZone  = this.a6Left.zone;  leftZone.syll = 'DA';
    const rightZone = this.a6Right.zone; rightZone.syll = 'DO';
  ...
  startA6Round() {
    ...
    const ROUNDS = [
      [ ['DA','dado'], ['DO','doce'] ],                                          // 2 pares  → 4 cartas
      [ ['DA','dama'], ['DE','dente'], ['DU','duende'] ],                        // 3 pares  → 6 cartas
      [ ['DI','dino'], ['DE','desenho'], ['DO','domino'], ['DU','ducha'] ],     // 4 pares  → 8 cartas
    ];
...
renderA8Round() {
  ...
  this.makeTitle('Ache a figura certa');
    ...
    this.a9Rounds = [
      { word:'DADO',    correct:'dado',    pool:['dama','dente','doce','dino'] },
      { word:'DAMA',    correct:'dama',    pool:['dado','dente','doce','dino'] },
      { word:'DENTE',   correct:'dente',   pool:['dado','dama','doce','dino'] },
      { word:'DOCE',    correct:'doce',    pool:['dado','dama','dente','dino'] },
      { word:'DINO',    correct:'dino',    pool:['dado','dama','dente','doce'] },
      { word:'DUENDE',  correct:'duende',  pool:['dado','dama','dente','doce'] },
    ];
    this.a9Index = 0;
    this._a9FirstRound = true;
    this.renderA9Round();
...
setupAtividade9() {
  ...
  // ordem pedagógica: DA -> DE -> DI -> DO -> DU
  ...
  this._a9Items = {
    DA: ['dado','dama','danca'],
    DE: ['dedo','dente','desenho'],
    DI: ['diamante','dinamite','dinheiro','dino'],
    DO: ['doce','doente','dois','domino','dormir'],
    DU: ['duende','ducha'],
  };
...
_a9RenderStep() {
  ...
  // Usado tanto pelo sistema de conclusão automática quanto pelo botão de teste
  goToActivityPalm(n) {
